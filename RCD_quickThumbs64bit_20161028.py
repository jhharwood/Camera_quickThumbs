...
#-------------------------------------------------------------------------------
# Name:        GSI_quickThumbs64bit_20130920.py
#
...
#               script will run either from a double click or running from command line after navigating to the correct folder.
#               If operating in an area where you have a three digit longitude and a negative symbol (-157.34545) a space
#               will have to be place between the latitdue and longitude for the CameraSync*_*.dat file.  This file is parsed by line using
#               a space delimiter.
#
#               The script creates a Thumbs directory to place resized/resampled jpgs for use in Google Earth.
#
...
#
# Change Log:
#       C. Macon; adjusted kml output - 20160905
#-------------------------------------------------------------------------------
...
for img in imgs:
    ...
    imgIn = img
    imgOut = imgName.replace('.tif','_scaled.jpg')
...
CCD_X = 0.054               # CCD_X = along track meters in m
CCD_Y = 0.040392            # CCD_Y = across track meters Left Wing + in m
...
# Open the CameraSync/coarse_lat_lon_ellHeight_roll_pitch_heading dat file and Event file
...
for iLine in ilist:
    ...
    if counter > 1:  # New Camera dat file has a system number header that allows chartsPic to see that it's an RCD30 camera
        ...
        iName = os.path.join(fileName + '_scaled.jpg')
        Lat = float(mylist[3])
        latF = format(Lat, '.11f')
        Lon = float(mylist[2])
        lonF = format(Lon, '.11f')
...
if os.name == "nt":
    for name in glob.glob("*scaled.jpg"):
